...
import { updateName } from 'src/store/reducers/user';
const me = {
  id: '',
  name: '',
  email: '',
    ...
    <Box backgroundColor={'white'} h={'100%'}>
      ...
      <Box mt={100} ml={5} mr={5} w={'90%'}>
        ...
        <FormControl w='100%' mt={5}>
          ...
          <Input isReadOnly variant='underlined' placeholder='Email' value={data.email} px={4} />
        </FormControl>

        {isEdit ? (
          <Box mt={'45px'}>
            <Button backgroundColor='blue.700' onPress={handleSave}>
              Save
            </Button>
          </Box>
        ) : (
