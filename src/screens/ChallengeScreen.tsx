...
import { getChallenges, getFriendChallenges } from 'src/store/reducers/challenges';
    ...
    <Box flex={1}>
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: '#d1d5db' }}>
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable onPress={() => handleViewDetail(item.id)}>
              <Box
                style={tw`p-5 mb-5`}
                backgroundColor='white'
                borderWidth='1'
              ...
              >
                <View style={tw`flex-row items-center gap-3 mb-3`}>
                  <Image
                    source={iconList[item.category as keyof typeof iconList].image}
                    alt='Category'
                    style={tw`w-10 h-10`}
                  />
                  <View>
                    ...
                    <Text
                      opacity={50}
                    >{`${formatter.format(item.date)}, ${item.date.getFullYear()}`}</Text>
                  </View>
                </View>
